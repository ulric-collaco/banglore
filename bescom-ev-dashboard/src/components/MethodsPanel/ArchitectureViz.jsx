...
const ARCH_LAYERS = [
  ...
  },
  {
    ...
    color: '#16a34a',
    items: ['Live Load Monitor — 24h heatmap, hex grid, coverage view', 'Infrastructure Planner — candidate selection with impact preview', 'Scheduling Advisor — per-station actionable recommendations', 'Algorithms Audit — full algorithmic transparency and baselines']
  },
  {
...
const EVAL_METRICS = [
  ...
  { metric: 'Explainability', method: 'All weights, factors, and coefficients visible in Algorithms panel', target: 'Full transparency', status: '✅ Achieved' },
...
const IMPLEMENTATION_PHASES = [
  ...
  { phase: 'Phase 4 — Evaluation & Hardening', duration: '2 weeks', items: ['Baseline comparison implementation', 'Algorithms audit panel for transparency', 'Risk mitigation documentation', 'Performance optimization'] }
];
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '32px' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>System Architecture</h3>
            ...
            <div key={layer.name} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <span style={{
                  ...
                  background: `${layer.color}14`, color: layer.color,
                  fontSize: '10px', fontWeight: 800, letterSpacing: '0.08em'
                }}>LAYER {i + 1}</span>
                <strong style={{ color: '#0f172a', fontSize: '14px' }}>{layer.name}</strong>
              </div>
        ...
        <div style={{ marginTop: '12px', padding: '10px 14px', borderRadius: '8px', background: 'rgba(22,163,74,0.06)', border: '1px solid rgba(22,163,74,0.18)' }}>
          <strong style={{ color: '#15803d', fontSize: '12px' }}>Key Design Decision:</strong>
      ...
      {/* Risk Assessment */}
      <div style={{ marginBottom: '32px' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Risk Assessment & Mitigation</h3>
              ...
              <tr key={r.risk}>
                ...
                <td>
                  <span style={{
                    ...
                    background: r.severity === 'High' ? 'rgba(239,68,68,0.1)' : r.severity === 'Medium' ? 'rgba(234,179,8,0.1)' : 'rgba(22,163,74,0.1)',
                    color: r.severity === 'High' ? '#dc2626' : r.severity === 'Medium' ? '#a16207' : '#15803d'
                  }}>{r.severity}</span>
                </td>
                <td style={{ color: '#64748b', fontSize: '12px', maxWidth: '260px' }}>{r.description}</td>
                <td style={{ fontSize: '12px', maxWidth: '300px', color: '#334155' }}>{r.mitigation}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Evaluation Metrics */}
      <div style={{ marginBottom: '32px' }}>
        <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Evaluation Approach</h3>
        <table className={styles.scoringTable}>
          <thead>
            <tr>
              <th>Metric</th>
          ...
          <tbody>
            {EVAL_METRICS.map((e) => (
              <tr key={e.metric}>
                ...
                <td style={{ color: '#64748b', fontSize: '12px' }}>{e.method}</td>
                <td style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '11px', color: '#334155' }}>{e.target}</td>
                <td style={{ color: '#15803d', fontWeight: 700 }}>{e.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Implementation Plan */}
      <div>
        <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>Implementation Plan</h3>
          ...
          {IMPLEMENTATION_PHASES.map((phase) => (
            <div key={phase.phase} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <strong style={{ color: '#0f172a', fontSize: '13px' }}>{phase.phase}</strong>
                <span style={{
                  padding: '2px 8px', borderRadius: '4px',
              ...
              <ul style={{ margin: 0, paddingLeft: '16px', color: '#64748b', fontSize: '12px', lineHeight: '1.6' }}>
